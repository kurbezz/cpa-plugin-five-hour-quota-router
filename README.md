...
- **Fast path for correlated active requests: response headers.** When CLIProxyAPI's after-auth hook correlates a successful Claude request's RequestID to its selected physical OAuth credential, the plugin observes Anthropic's `Anthropic-Ratelimit-Unified-5h-*` response headers and updates that credential's cached five-hour sample immediately. `Anthropic-Ratelimit-Unified-5h-Utilization` is a fraction (`0.23` = 23%) and is converted to the plugin's 0-100 percent scale; `Anthropic-Ratelimit-Unified-5h-Status: rejected` is treated as 100%. A missing correlation, unknown/replaced credential, or unsafe exhausted header with no reset is ignored. **This only observes successful responses** — a 429/error response from Anthropic is not observed this way.
- **Reliable startup/idle/fallback source: the `/api/oauth/usage` poll.** Refreshes enabled physical Claude OAuth credentials when the worker starts; startup reconfiguration retries discovery only while the cache is empty. There is no time-driven polling loop. When a protected-model request selects an account whose cached usage is at least `poll-interval` old, queues one asynchronous refresh for that account while routing the current request from memory. This remains the reliable fallback whenever no response-header correlation succeeds, including accounts that have never had a real successful request routed through them.
