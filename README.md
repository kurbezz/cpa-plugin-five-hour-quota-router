...
- **Primary source for active accounts: response headers.** Every real, successful (non-error) Claude upstream response — streaming or not — carries Anthropic's `Anthropic-Ratelimit-Unified-5h-*` rate-limit headers. The plugin observes these via CLIProxyAPI's response and stream-chunk interceptor hooks and updates the credential's cached five-hour sample immediately, with no polling delay. This keeps active accounts' quota state current between usage-API polls instead of lagging by up to `poll-interval`. `Anthropic-Ratelimit-Unified-5h-Utilization` is a fraction (`0.23` = 23%) and is converted to the plugin's 0-100 percent scale; `Anthropic-Ratelimit-Unified-5h-Status: rejected` is treated as 100% regardless of (or absent) a utilization value. **This only observes successful responses** — a 429/error response from Anthropic is not observed this way (CLIProxyAPI's response interceptors only run on success), so an account already fully blocked relies on the usage-API poll or the scheduler backstop to detect it, not this header path.
- **Fallback/idle source: the `/api/oauth/usage` poll.** Refreshes enabled physical Claude OAuth credentials when the worker starts; startup reconfiguration retries discovery only while the cache is empty. There is no time-driven polling loop. When a protected-model request selects an account whose cached usage is at least `poll-interval` old, queues one asynchronous refresh for that account while routing the current request from memory. This remains the only source for an account that has never yet had a real successful request routed through it, and continues to run alongside header observation for every account.
